...
void RenderInterface_VK::BeginFrame()
{
	...
	// Pipeline uses VK_DYNAMIC_STATE_SCISSOR — must set a default scissor covering the
	// full framebuffer before any draw calls. Individual scissor regions are set later
	// via EnableScissorRegion / SetScissorRegion.
	VkRect2D default_scissor = {{0, 0}, {(uint32_t)m_width, (uint32_t)m_height}};
	vkCmdSetScissor(m_p_current_command_buffer, 0, 1, &default_scissor);
...
int RenderInterface_VK::Choose_SwapchainImageCount(uint32_t user_swapchain_count_for_creation, bool if_failed_choose_min) noexcept
{
	...
	RMLUI_VK_ASSERTMSG(caps.maxImageCount == 0 || user_swapchain_count_for_creation <= caps.maxImageCount,
		"can't be, you must have a valid count that bounds from minImageCount to maxImageCount! Otherwise you will get a validation error that "
		"specifies that you created a swapchain with invalid image count");

	int result = 0;

	// maxImageCount == 0 means "no upper limit" per Vulkan spec (common on Wayland)
	if (user_swapchain_count_for_creation < caps.minImageCount || (caps.maxImageCount != 0 && user_swapchain_count_for_creation > caps.maxImageCount))
		result = if_failed_choose_min ? caps.minImageCount : (caps.maxImageCount != 0 ? caps.maxImageCount : user_swapchain_count_for_creation);
	else
...
void RenderInterface_VK::Wait() noexcept
{
	...
	// Wait for the PREVIOUS frame's fence: ensures frame N-1's GPU work is complete
	// before we start frame N (sequential single-buffer pipelining).
	...
	// Reset the CURRENT slot's fence so it is unsignaled before Submit() uses it.
	// The original code reset fences[prev] instead, leaving fences[curr] pre-signaled
	// and causing a validation error on the first two frames.
...
#if !defined(RMLUI_VK_NO_VMA_IMPLEMENTATION)
