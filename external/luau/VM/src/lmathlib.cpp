...
static float lerp_iternal(float t, float a, float b)
{
    return a + t * (b - a);
}
...
static float perlin(float x, float y, float z)
{
    ...
    return lerp_iternal(w,
        lerp_iternal(v, lerp_iternal(u, grad(p[aa], xf, yf, zf), grad(p[ba], xf - 1, yf, zf)),
            lerp_iternal(u, grad(p[ab], xf, yf - 1, zf), grad(p[bb], xf - 1, yf - 1, zf))),
        lerp_iternal(v, lerp_iternal(u, grad(p[aa + 1], xf, yf, zf - 1), grad(p[ba + 1], xf - 1, yf, zf - 1)),
            lerp_iternal(u, grad(p[ab + 1], xf, yf - 1, zf - 1), grad(p[bb + 1], xf - 1, yf - 1, zf - 1))));
}
