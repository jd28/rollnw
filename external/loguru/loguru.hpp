...
LOGURU_EXPORT
    Text
    textprintf(LOGURU_FORMAT_STRING_TYPE format, const Args&... args)
{
...
template <typename... Args>
LOGURU_EXPORT void log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args)
{
...
template <typename... Args>
LOGURU_EXPORT void raw_log(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args)
{
...
template <typename... Args>
LOGURU_EXPORT
    LOGURU_NORETURN void
    log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args)
{
...
#define VLOG_SCOPE_F(verbosity, ...) \
    loguru::LogScopeRAII LOGURU_ANONYMOUS_VARIABLE(error_context_RAII_) = ((verbosity) > loguru::current_verbosity_cutoff()) ? loguru::LogScopeRAII() : loguru::LogScopeRAII(verbosity, __FILE__, __LINE__, __VA_ARGS__)
...
#define CHECK_WITH_INFO_F(test, info, ...) \
    LOGURU_PREDICT_TRUE((test) == true) ? (void)0 : loguru::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__, __LINE__, ##__VA_ARGS__)
