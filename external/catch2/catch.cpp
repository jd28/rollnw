...
int Session::runInternal()
{
    ...
    CATCH_TRY
    {
        ...
        if (totals.testCases.total() > 0 && totals.testCases.total() == totals.testCases.skipped
            && !m_config->zeroTestsCountAsSuccess()) {
            return 4;
        }
...
void Parser::writeToStream(std::ostream& os) const
{
    ...
    for (auto const& cols : rows) {
        auto row = TextFlow::Column(cols.left)
                       .width(optWidth)
                       .indent(2)
            + TextFlow::Spacer(4) + TextFlow::Column(cols.right).width(consoleWidth - 7 - optWidth);
        os << row << '\n';
    }
}
...
Clara::Parser makeCommandLineParser(ConfigData& config)
{
    ...
    auto const setDefaultColourMode = [&](std::string const& colourMode) {
        ...
        if (!isColourImplAvailable(mode)) {
            return ParserResult::runtimeError(
                "colour mode '" + colourMode + "' is not supported in this binary");
        }
    ...
    auto const setReporter = [&](std::string const& userReporterSpec) {
        ...
        if (result == factories.end()) {
            return ParserResult::runtimeError(
                "Unrecognized reporter, '" + reporterSpec.name() + "'. Check available with --list-reporters");
        }
...
std::vector<std::string> splitReporterSpec(StringRef reporterSpec)
{
    ...
    auto findNextSeparator = [&reporterSpec](size_t startPos) {
        ...
        do {
            while (currentPos < reporterSpec.size() && reporterSpec[currentPos] != separator[0]) {
                ++currentPos;
            }
            if (currentPos + 1 < reporterSpec.size() && reporterSpec[currentPos + 1] == separator[1]) {
                return currentPos;
            }
            ++currentPos;
...
{
    return lhs.m_name == rhs.m_name && lhs.m_outputFileName == rhs.m_outputFileName && lhs.m_colourMode == rhs.m_colourMode && lhs.m_customOptions == rhs.m_customOptions;
}
    ...
    void close() override
    {
        ...
        const bool should_wait_for_child = [&]() {
            ...
            for (auto const& child : m_children) {
                if (child->isSectionTracker() && std::find(filters.begin(), filters.end(), static_cast<SectionTracker const&>(*child).trimmedName()) != filters.end()) {
                    return true;
                }
            }
            return false;
        ...
        if (should_wait_for_child || (m_runState == CompletedSuccessfully && m_generator->countedNext())) {
            m_children.clear();
    ...
    Generators::GeneratorBasePtr&& generator)
{
    ...
    assert(
        currentTracker.nameAndLocation() != nameAndLoc && "Trying to create tracker for a genreator that already has one");
...
namespace Catch {
TestCaseInfoHasher::TestCaseInfoHasher(hash_t seed)
    : m_seed(seed)
{
}
        ...
        std::sort(indexed_tests.begin(),
            ...
            [](TestWithHash const& lhs, TestWithHash const& rhs) {
                if (lhs.first == rhs.first) {
                    return lhs.second.getTestCaseInfo() < rhs.second.getTestCaseInfo();
                }
...
void enforceNoDuplicateTestCases(std::vector<TestCaseHandle> const& tests)
{
...
void Column::const_iterator::calcLength()
{
    ...
    if (m_parsedTo < m_lineStart + maxLineLength) {
        ...
        while (newLineLength > 0 && isWhitespace(current_line[m_lineStart + newLineLength - 1])) {
            --newLineLength;
        }
...
{
    auto initial = m_lineStart == 0 ? m_column.m_initialIndent : std::string::npos;
...
Column::const_iterator& Column::const_iterator::operator++()
{
    ...
    if (m_lineStart < current_line.size() && current_line[m_lineStart] == '\n') {
        ...
        while (m_lineStart < current_line.size() && isWhitespace(current_line[m_lineStart])) {
            ++m_lineStart;
        }
    }
...
XmlFormatting operator|(XmlFormatting lhs, XmlFormatting rhs)
{
    return static_cast<XmlFormatting>(
        static_cast<std::underlying_type_t<XmlFormatting>>(lhs) | static_cast<std::underlying_type_t<XmlFormatting>>(rhs));
}
    ...
    AssertionStats const& assertionStats)
{
    ...
    if (m_shouldStoreFailedAssertions && !assertionStats.assertionResult.isOk()) {
        static_cast<void>(
...
std::string fileNameTag(std::vector<Tag> const& tags)
{
    auto it = std::find_if(begin(tags),
        ...
        [](Tag const& tag) { return tag.original.size() > 0
                                 && tag.original[0] == '#'; });
    if (it != tags.end()) {
