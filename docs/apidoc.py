...
NESTED_TYPES = ["enum", "typedef", "define", "function"]
...
def create_package_file(package, package_type, package_id, args):
    ...
    outfile = package.replace(':', '_')

    write_file(os.path.join(package_type, outfile), text, args)
    ...
    for compound in index.getroot():
        if compound.get("kind") == "namespace":
            for member in compound:
                if member.get("kind") in NESTED_TYPES:
                    create_package_file(
                        f'{compound.findtext("name")}::{member.findtext("name")}',
                        member.get("kind"), member.get("refid"), args
                    )
        elif compound.get("kind") == "file":
            name = compound.find("name")
            if name.text.endswith('.cpp'):
                continue

            for member in compound:
                if member.get("kind") == "define":
                    create_package_file(member.findtext("name"),
                                        member.get("kind"), member.get(
                                            "refid"), args
                                        )
        else:
            create_package_file(
                compound.findtext("name"), compound.get(
                    "kind"), compound.get("refid"), args
            )
