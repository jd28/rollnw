...
ParticleBillboardAxes resolve_particle_billboard_axes(const RenderContext& ctx, const ParticleSystemInstance& system,
    const ParticleRenderPacket& packet, uint32_t particle_index, const ParticleBillboardAxes& fallback) noexcept
{
    ...
    if (packet.mode == ParticleRenderMode::billboard_world_z) {
        ...
        axis_billboard_axes(ctx, position, glm::vec3{0.0f, 0.0f, 1.0f}, axes.right, axes.up);
    }
    ...
    for (const auto& packet : packets) {
        ...
        if (packet.mode != ParticleRenderMode::linked_chain
            ...
            for (uint32_t ordered_index = 0; ordered_index < order_.size(); ++ordered_index) {
                ...
                const ParticleBillboardAxes packet_axes = resolve_particle_billboard_axes(ctx, system, packet,
