...
float4 main(PSInput input) : SV_Target {
    ...
    if (two_sided_lighting != 0 && dot(geom_N, V) < 0.0) {
        geom_N = -geom_N;
        T = -T;
        B = -B;
    }
    float3 N = material_normal(geom_N, T, B, input.texcoord, material_texture_indices.x);
    ...
    float roughness = clamp(material_params.x, 0.05, 1.0);
    float specular_strength = max(material_params.y, 0.0);
    roughness = material_roughness(roughness, input.texcoord, material_texture_indices.y);
    specular_strength = material_specular_strength(specular_strength, input.texcoord, material_texture_indices.w);
    ...
    if (material_texture_indices.z != 0u) {
        final_color += g_textures[NonUniformResourceIndex(material_texture_indices.z)].Sample(g_sampler, input.texcoord).rgb
            * emissive_color.rgb;
    } else {
        final_color += albedo * emissive_color.rgb;
    }
