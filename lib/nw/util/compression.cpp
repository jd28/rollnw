...
inline ByteArray zlib_decompress(absl::Span<const std::byte> span, uint32_t uncompressed_size)
{
    ByteArray result;
...
inline ByteArray zstd_decompress(absl::Span<const std::byte> span, uint32_t uncompressed_size)
{
...
ByteArray decompress(absl::Span<const std::byte> span, const char* magic)
{
    ...
    memcpy(&hdr, span.data(), hdr_sz);
    span = absl::Span<const std::byte>{span.data() + hdr_sz, span.size() - hdr_sz};
