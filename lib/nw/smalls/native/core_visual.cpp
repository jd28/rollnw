...
void register_core_visual(Runtime& rt)
{
    ...
    rt.module("core.visual")
        ...
        .function("set_visual_hold_animation", +[](nw::ObjectHandle obj, nw::Resref animation) -> bool {
            if (!valid_object(obj) || animation.empty()) { return false; }
            return nw::kernel::objects().components().set_visual_hold_animation(obj, animation); })
        ...
        .function("add_visual_model_row", +[](nw::ObjectHandle obj, nw::Resref model, nw::Resref plt_texture, nw::Resref attach_to, nw::Resref attach_from, int32_t kind, int32_t slot, int32_t part, int32_t source_part, int32_t model_part, int32_t flags) -> bool {
        ...
        .function("clear_item_icons", +[](nw::ObjectHandle obj) -> bool {
            if (!valid_object(obj) || obj.type != nw::ObjectType::item) { return false; }
            return nw::kernel::objects().components().clear_item_icons(obj); })
        .function("add_item_icon_layer", +[](nw::ObjectHandle obj, int32_t variant, nw::Resref resource, nw::Resref fallback, int32_t part, int32_t flags, int32_t metal1, int32_t metal2, int32_t cloth1, int32_t cloth2, int32_t leather1, int32_t leather2) -> bool {
            if (!valid_object(obj) || obj.type != nw::ObjectType::item
                || variant < 0 || variant >= nw::ObjectItemIconState::variant_count
                || flags < 0 || flags > std::numeric_limits<uint8_t>::max()) {
                return false;
            }
            nw::PltColors colors{};
            if (!set_plt_color(colors, nw::plt_layer_metal1, metal1)
                || !set_plt_color(colors, nw::plt_layer_metal2, metal2)
                || !set_plt_color(colors, nw::plt_layer_cloth1, cloth1)
                || !set_plt_color(colors, nw::plt_layer_cloth2, cloth2)
                || !set_plt_color(colors, nw::plt_layer_leather1, leather1)
            ...
            return nw::kernel::objects().components().add_item_icon_layer(
                obj, static_cast<uint8_t>(variant), nw::ObjectItemIconLayer{
                    .resource = resource,
                    .fallback = fallback,
                    .plt_colors = colors,
                    .part = part,
