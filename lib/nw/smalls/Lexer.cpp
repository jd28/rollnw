...
#define NSS_TOKEN(type, size)                             \
    do {                                                  \
        ...
        pos_ += size;                                     \
        t.loc.range.end = {line_, pos_ - last_line_pos_}; \
    } while (0)
    ...
    while (pos_ < buffer_.size()) {
        ...
        case 'R': {
            if (kernel::config().version() == GameVersion::vEE
                ...
                if (!matched) {
                    ...
                    loc.range.end = {start_pos.line, start_pos.column + 2};
        ...
        case '"': {
            ...
            if (pos_ == buffer_.size() || get(pos_) != '"') {
                ...
                loc.end = buffer_.data() + start;
                loc.range.start = start_pos;
                loc.range.end = {start_pos.line, start_pos.column + 1};
        ...
        case '&': // AND
            ...
            case '&':
                ...
                break;
            default:
                ctx_->lexical_diagnostic(parent_, fmt::format("Unrecognized character '{}'", get(pos_)), true,
                    {{line_, start - last_line_pos_}, {line_, start - last_line_pos_ + 1}});
                ++pos_;
                continue;
            }
            break;
        ...
        case '*': // TIMES
            ...
            case '/': { // Uh oh
                ...
                SourceLocation loc{
                    loc_start,
                    buffer_.data() + pos_ + 1,
                    {start_pos, {line_, pos_ + 1 - last_line_pos_}},
                };
