...
toolset SmallS = remaining non-appearance presentation adapters
...
- finish moving non-Item widget markup still generated by C++ into RML; or
...
those batches into one RmlUi data model; static selector structure and event
wiring live in RML. The managed-list host remains the bounded path for long
option lists.
...
- the remaining non-appearance adapters in
...
RCSS contains their layout. Native RmlUi callbacks dispatch appearance events;
toolset SmallS retains only the other narrow presentation adapters.
`nwn1.item` produces policy rows and publishes resolved visual rows; it owns no
editor state.
...
- [x] Move Item editor rows and interaction state to the native Item editor;
      retain only policy-row production in `nwn1.item`.
- [x] Replace complete SmallS/C++-generated Item appearance markup with one
      native RmlUi data model and static RML event wiring.
