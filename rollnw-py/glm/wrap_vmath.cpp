...
void wrap_Vector3(py::module_& m)
{
    ...
    t.def_rw("x", &glm::vec3::x);
    t.def_rw("y", &glm::vec3::y);
    t.def_rw("z", &glm::vec3::z);
...
void wrap_Quaternion(py::module_& m)
{
    ...
    t.def_rw("w", &glm::quat::w);
    t.def_rw("x", &glm::quat::x);
    t.def_rw("y", &glm::quat::y);
    t.def_rw("z", &glm::quat::z);
...
void wrap_Matrix4(py::module_& m_)
{
    ...
    t.def("__init__", [](float m00, float m01, float m02, float m03, float m10, float m11, float m12, float m13, float m20, float m21, float m22, float m23, float m30, float m31, float m32, float m33) {
        return glm::mat4(
            m00, m01, m02, m03,
            m10, m11, m12, m13,
            m20, m21, m22, m23,
    ...
    t.def_prop_rw("m00", [](glm::mat4& m) { return m[0][0]; }, [](glm::mat4& m, float f) { m[0][0] = f; });
    t.def_prop_rw("m01", [](glm::mat4& m) { return m[0][1]; }, [](glm::mat4& m, float f) { m[0][1] = f; });
    t.def_prop_rw("m02", [](glm::mat4& m) { return m[0][2]; }, [](glm::mat4& m, float f) { m[0][2] = f; });
    t.def_prop_rw("m03", [](glm::mat4& m) { return m[0][3]; }, [](glm::mat4& m, float f) { m[0][3] = f; });

    t.def_prop_rw("m10", [](glm::mat4& m) { return m[1][0]; }, [](glm::mat4& m, float f) { m[1][0] = f; });
    t.def_prop_rw("m11", [](glm::mat4& m) { return m[1][1]; }, [](glm::mat4& m, float f) { m[1][1] = f; });
    t.def_prop_rw("m12", [](glm::mat4& m) { return m[1][2]; }, [](glm::mat4& m, float f) { m[1][2] = f; });
    t.def_prop_rw("m13", [](glm::mat4& m) { return m[1][3]; }, [](glm::mat4& m, float f) { m[1][3] = f; });

    t.def_prop_rw("m20", [](glm::mat4& m) { return m[2][0]; }, [](glm::mat4& m, float f) { m[2][0] = f; });
    t.def_prop_rw("m21", [](glm::mat4& m) { return m[2][1]; }, [](glm::mat4& m, float f) { m[2][1] = f; });
    t.def_prop_rw("m22", [](glm::mat4& m) { return m[2][2]; }, [](glm::mat4& m, float f) { m[2][2] = f; });
    t.def_prop_rw("m23", [](glm::mat4& m) { return m[2][3]; }, [](glm::mat4& m, float f) { m[2][3] = f; });

    t.def_prop_rw("m30", [](glm::mat4& m) { return m[3][0]; }, [](glm::mat4& m, float f) { m[3][0] = f; });
    t.def_prop_rw("m31", [](glm::mat4& m) { return m[3][1]; }, [](glm::mat4& m, float f) { m[3][1] = f; });
    t.def_prop_rw("m32", [](glm::mat4& m) { return m[3][2]; }, [](glm::mat4& m, float f) { m[3][2] = f; });
    t.def_prop_rw("m33", [](glm::mat4& m) { return m[3][3]; }, [](glm::mat4& m, float f) { m[3][3] = f; });
...
void wrap_Transform(py::module_& m)
{
    ...
    t.def_rw("translation", &Transform::translation);
    t.def_rw("rotation", &Transform::rotation);
    t.def_rw("scale", &Transform::scale);
...
void wrap_Vector2(py::module_& m)
{
    ...
    t.def_rw("x", &glm::vec2::x);
    t.def_rw("y", &glm::vec2::y);
...
void wrap_Vector4(py::module_& m)
{
    ...
    t.def_rw("x", &glm::vec4::x);
    t.def_rw("y", &glm::vec4::y);
    t.def_rw("z", &glm::vec4::z);
    t.def_rw("w", &glm::vec4::w);
...
void wrap_Ray(py::module_& m)
{
    ...
    t.def_rw("position", &Ray::position);
    t.def_rw("direction", &Ray::direction);
