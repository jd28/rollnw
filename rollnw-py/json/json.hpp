...
inline py::object from_json(const nl::json& j)
{
    ...
    } else // Object
    {
        ...
        for (nl::json::const_iterator it = j.cbegin(); it != j.cend(); ++it) {
            auto s = it.key();
            obj[py::str(s.c_str())] = from_json(it.value());
        }
        return obj;
    }
}
...
inline nl::json to_json(const py::handle& obj)
{
    ...
    if (py::isinstance<py::int_>(obj)) {
        ...
        throw std::runtime_error("to_json received an integer out of range for both nl::json::number_integer_t and nl::json::number_unsigned_t type: " + py::cast<std::string>(py::repr(obj)));
    }
    ...
    throw std::runtime_error("to_json not implemented for this type of object: " + py::cast<std::string>(py::repr(obj)));
}
}
